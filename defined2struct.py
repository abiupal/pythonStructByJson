#!/usr/bin/env python
# -*- coding: utf-8 -*-
""" Read binary file as C Header defined struct.
* Using reading binary file class.

Todo:
    * Using output.json by struct2json.py
...
def read(defined, target_key, bytes_data, offset):
    ...
    # 解析対象のバイナリ
    # 解析開始位置
    """ read struct defined JSON file
    Args:
        defined (string): structure defined
        target_key (string): structure member name
        bytes_data(bytes): read binary data
        offset (number): start to read data offset
    """
